...
with st.container(border=True):
    st.markdown(
        "**⌨️ Escribe el nombre de la ciudad** — las sugerencias se buscan en tiempo real "
        "recorriendo un Trie (árbol de prefijos), letra por letra, mientras escribes."
    )
    col1, col2 = st.columns(2)

    with col1:
        texto_origen = st.text_input(
            "📍 Origen", key="texto_origen", placeholder="Ej: Miami, Atlanta, Columbus..."
        )
        if texto_origen:
            sugerencias_origen = trie_ciudades.autocompletar(texto_origen)
            if sugerencias_origen:
                elegido_origen = st.radio(
                    "Sugerencias (en vivo):", sugerencias_origen,
                    key="radio_origen", label_visibility="collapsed",
                )
                st.session_state["sel_origen"] = elegido_origen
            else:
                st.caption("🔸 Sin coincidencias todavía con lo que escribiste.")
        ciudad_ori = st.session_state.get("sel_origen")
        if ciudad_ori:
            st.caption(f"✅ Origen seleccionado: **{ciudad_ori}**")

    with col2:
        texto_destino = st.text_input(
            "🏁 Destino", key="texto_destino", placeholder="Ej: Portsmouth, Denver, Seattle..."
        )
        if texto_destino:
            sugerencias_destino = trie_ciudades.autocompletar(texto_destino)
            if sugerencias_destino:
                elegido_destino = st.radio(
                    "Sugerencias (en vivo):", sugerencias_destino,
                    key="radio_destino", label_visibility="collapsed",
                )
                st.session_state["sel_destino"] = elegido_destino
            else:
                st.caption("🔸 Sin coincidencias todavía con lo que escribiste.")
        ciudad_des = st.session_state.get("sel_destino")
        if ciudad_des:
            st.caption(f"✅ Destino seleccionado: **{ciudad_des}**")
    ...
    for registro in st.session_state["mis_rutas"]:
        ...
        with col_editar:
            if st.button("✏️", key=f"editar_{registro['id']}", help="Editar esta ruta"):
                ...
                st.session_state["input_alias"] = registro["alias"]
                st.session_state["input_origen_manual"] = registro["origen"]
                st.session_state["input_destino_manual"] = registro["destino"]
                st.session_state["input_notas"] = registro["notas"]
                st.rerun()
        with col_eliminar:
